...
const SingleItem = ({
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg bg-[#F6F7FB] aspect-[3/4]">
      {/* Full image */}
      <img
      ...
      {/* Rank number */}
      <span className="absolute top-4 left-4 text-[64px] font-bold leading-none text-white/20 select-none">
        {String(rank).padStart(2, "0")}
      </span>

      {/* Discount badge */}
      {discount > 0 && (
        <span className="absolute top-4 right-4 bg-blue text-white text-xs font-medium px-2.5 py-1 rounded-full">
          -{discount}%
        </span>
      )}

      {/* Top-right action buttons - fade in on hover */}
      <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 translate-x-3 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0"
        style={discount > 0 ? { top: "3rem" } : {}}
      >
        <button
          onClick={() => {
          ...
          className="flex items-center justify-center w-9 h-9 rounded-full bg-white/90 backdrop-blur-sm text-dark shadow-1 ease-out duration-200 hover:bg-dark hover:text-white"
        >
          <svg
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-dark/80 via-dark/40 to-transparent pt-20 pb-5 px-5">
          ...
          <div className="flex items-center gap-0.5">
            {[...Array(5)].map((_, i) => (
              <svg
                ...
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M7 0.5L8.91 4.86L13.5 5.27L10 8.39L10.94 13L7 10.64L3.06 13L4 8.39L0.5 5.27L5.09 4.86L7 0.5Z" />
              </svg>
            ))}
          </div>
          <span className="text-xs text-white/70">({item.reviews})</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm font-medium text-white bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full ease-out duration-200 hover:bg-white hover:text-dark text-nowrap"
          >
            <svg
              className="fill-current"
              width="14"
